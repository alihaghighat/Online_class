...
if (isset($_REQUEST['idclass'])) {
    ...
    $temp = array("name" => $przenter['name'], "kind" => "P", "id" => 0, "lastname" => $przenter['lastname'], "mic" => 1, "Wbord" => 5);
    array_push($array, $temp);
    $getclassstudent = getrecord("tblstudtClass", "idclass=$idclass");
    foreach ($getclassstudent as $key) {
        $idOnline = $key['idstudtClass'];
        $getOnline = getrecord("tblstudtClassOnline", "idstudtClass=$idOnline  order by id desc");
        if (count($getOnline) > 0) {
            ...
            if ($getOnline[0]['time_end'] == 0) {
                ...
                $temp = $temp = array("name" => $Onlinestudent['name'], "kind" => "L", "id" => $id, "lastname" => $Onlinestudent['lastname'], "mic" => $mic, "Wbord" => $Wboard);
                array_push($array, $temp);
            } else {
    ...
    foreach ($array as $key) {
        ...
        if ($key['mic'] == 1) {
            $data = $data . '<div class="member-access">
                <div class="access-place p-2 rounded" onclick="mic1(' . $key['id'] . ')">
                    <i class="bi-mic access-button" ></i>
                </div>
            </div>
            ';
        }
        if ($key['mic'] == 0) {
            $data = $data . '<div class="member-access">
                <div class="access-place p-2 rounded" onclick="mic1(' . $key['id'] . ')">
                    <i class="bi-mic-mute access-button" ></i>
                </div>
            </div>
            ';
        }
        if ($key['Wbord'] == 0) {
            $data = $data . '<div class="member-access">
                <div class="access-place p-2 rounded" onclick="setHost(' . $key['id'] . ')">
                   <i class="bi-cast access-button"></i>
                </div>
            </div>
            ';
        }
        if ($key['Wbord'] == 1) {
            $data = $data . '<div class="member-access">
                <div class="access-place p-2 rounded" onclick="setHost(' . $key['id'] . ')">
                   <i class="bi-x-square access-button"></i>
                </div>
            </div>
            ';
        }
